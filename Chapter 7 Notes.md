...
Use `int()` function to convert the string into a number type
```python
age = input('How old are you? ')
age = int(age)
...
* If the message is not set at the beginning, Python has nothing to compare, which cannot run the program
